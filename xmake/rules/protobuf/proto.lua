...
function _get_grpc_cpp_plugin(target, sourcekind)
    ...
    grpc_cpp_plugin = find_tool("grpc_cpp_plugin", {norun = true})
...
function buildcmd(target, batchcmds, sourcefile_proto, opt, sourcekind)
    ...
    if grpc_cpp_plugin then
        batchcmds:set_depmtime(os.mtime(objectfile_grpc))
        batchcmds:set_depcache(target:dependfile(objectfile_grpc))
    end
end
